    ...
    public void handle(String target, Request baseRequest, HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        try {
            ...
            for (Integer gameId : user2GameIds) {
                ...
                VersusResult result = new VersusResult(new SteamGame(game.getInteger("id").intValue(), game.getString("icon_url"), game.getString("logo_url"), game.getString("name")), user1Games.get(
                        i).getFloat("completion_rate"), user2Games.get(j).getFloat("completion_rate"));
